...
exports.findAndExecute = function(message, command) {
    var commandModule = exports.findCommand(command);
    ...
    if(validityMessage !== true) {
        if(typeof validityMessage === "string") {
            message.channel.sendMessage(validityMessage);
        }
        return;
    }
    ...
    try {
        ...
        message.channel.sendMessage("An error occurred while processing the " +
            `command \`\`\`\n${command}\n\`\`\`\n: \n\`\`\`\n${err}\n\`\`\``);
    }
};
